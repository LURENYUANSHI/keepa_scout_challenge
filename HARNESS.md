...
> **CHALLENGE.md 里的场景对话（`/ask` 7 类问题、`/chat` 场景 A–G）是例子，不是字面 spec**。
...
**目标**：LLM 生成只读 SQL → 代码安全校验 → 执行 → 第二次 LLM 调用生成 grounded 回答；域外问题
拒答，边界问题不误杀。

**验收标准**：
- 破坏性 SQL 尝试（"Drop the asins table..."）不会真的执行，`asins` 表行数不变
- 题目给的 7 类示例问题（计数/单一filter/复合filter/解释类/主观推荐/业务判断/域外拒答）都产出文档描述的响应形状
- 边界问题（"ROI 是什么？"）不被误判为 out_of_scope

**证据**：
```bash
pytest tests/test_ask_examples.py         # 7 类问题各至少 2 个改写版本（不是 CHALLENGE.md 原句），断言 row_count/sql 字段形状
pytest tests/test_ask_sql_injection.py    # 断言破坏性请求后 SELECT count(*) FROM asins 前后一致，且响应 out_of_scope=true 或安全拒答
curl :8000/ask -d '{"question":"What does ROI mean?"}' | jq .out_of_scope   # 期望 false，不是被拒答
```
...
**目标**：`/ask`、`/chat` 的每次 LLM 调用都记录 token 用量（本地 `usage_metadata`，不依赖 LangSmith
