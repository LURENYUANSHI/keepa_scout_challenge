...
写死一个会过期的数字）。**Keepa token 消耗为 0**——开发环境网络无法访问 `api.keepa.com`
（详见下方"故意没做好的地方"），所有 Keepa 交互走的是 mock/合成数据，没有真实调用，因此也
没有消耗真实 Keepa 额度。两个 Keepa key 共享同一个 token 池（约 600 + 5/min 回血），非独立
额度，多 key 轮换解决的是限流问题而不是扩大总额度。
...
- **本地开发/验证环境连不上 `api.keepa.com`**（DNS 解析到网络测试用的哨兵地址）——不是代码
  问题，是这次开发所在沙箱的网络限制。所有 Keepa 相关的验证走的是 respx mock（单测）或者灌
  进真实 ETL pipeline 的合成 fixture 数据（集成测试/演示用）。提交前需要在有正常网络的环境
  跑一遍真实 `python -m app.etl` 和 `/upc` 确认真实数据路径

## AI 工具坦白

本项目使用 **Claude Code**（Anthropic，模型 Claude Sonnet 5）完成，采用**多子 agent 编排**的
