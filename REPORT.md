...
- **开发过程中大部分时间连不上 `api.keepa.com`**（DNS 一度解析到网络测试用的哨兵地址）——
  ...
  （32/32 成功）、以及多轮真实 `scripts/verify_upc.py`。**最终在一个出网窗口内跑出了单轮
  完整判定**（`phase7/verify_upc_clean_run.txt`）：4 个 required 全部 PASS（含 Bookland
  ISBN 的 case_03 → `054546529X`）、两个加分项也 PASS（`hard` GTIN-14 补零的 case_04 →
  `122308079X`；带横杠脏输入的 case_07）、case_05 返回 3 个 ASIN 印证"同一 UPC 多 listing
  全部返回"、case_06 全 9 假条码按设计返回空。抖动的直观证据也留了档：同晚相隔几分钟的
  两轮运行一轮过 case_01–04、另一轮只过 case_07（`phase7/verify_upc_rerun_2300.txt`）——
  同一份代码，结果完全跟着出网窗口走。网络仍不视为"已解决"，Loom 现场演示 /upc 前建议先
  探连通性
