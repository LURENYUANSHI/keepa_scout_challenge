...
> **对 CHALLENGE.md 的两处偏离，先说清楚**：
>
> ① 题目给的 `/upc`、`/eligibility`、`/chat`、`/refresh` 等 curl 示例都不带 `Authorization`
> header。本设计决定加**强制鉴权**、**不做匿名回退**——所有业务端点都必须先
> `POST /auth/register` + `POST /auth/login` 拿到 token 才能调用。README 里的 curl 示例会
> 相应地在最前面加一步"注册/登录拿 token"。
>
> ② **移除了 `POST /ask`**（题目必做端点之一，属主动偏离，人为拍板）。理由：作为题目要求
> 无可厚非，但对本系统的功能与架构而言它是冗余的——`/chat` 的 agent 经 `run_readonly_sql`
> 工具覆盖了完全相同的 NL→SQL→安全校验→执行→grounded 回答能力，且更强（多轮、状态、偏好）。
> 保留两条并行的 NL→SQL 管线只是为了满足题面字数。SQL 安全校验（SELECT-only/单语句/禁
> DDL-DML）不受影响——它一直只有一处定义（`validate_readonly_sql`），如今由 `/chat` 的工具
> 入口独占使用。题目要求的 7 类 `/ask` 示例问题在 README 里全部改经 `/chat` 演示。
...
erDiagram
    ...
    LLM_USAGE_LOG {
        bigserial id PK
        uuid user_id FK
        text session_id "可空——历史上无 session 的 /ask 调用（端点已移除）"
        text endpoint "chat / chat_stream（历史行可为 ask）"
        text model
        int input_tokens
        int output_tokens
        int total_tokens
...
- 偏好数据 **Store 是唯一数据源**——不做 SQL 可查的镜像表，避免两个来源打架；偏好类问题
  （"我的预算是多少"）只经 `/chat` 由 agent 通过 Store 回答
...
`/eligibility/{asin}`、`/eligibility/batch`、`/chat`、`WS /chat/stream`
...
**`/ask` 的最终去向（已移除——文档开头"偏离②"）**：这个端点经历了两轮决策。第一轮，代码
审计建议"删除冗余的 /ask"被驳回，理由是它是题目必做端点，实现为独立的轻量两次-LLM-调用管线
（不走 agent 图，只 import 同一份 `validate_readonly_sql`/`run_readonly_sql_impl`）。第二轮
（提交收尾阶段）人为推翻了第一轮：题面合规让位于架构判断——两条并行的 NL→SQL 管线里，
`/ask` 那条是 `/chat` 能力的严格子集（无多轮、无状态、无偏好），保留它只是在为题面字数付
维护成本，最终整体移除（路由/schema/测试一并删除）。移除不影响安全面："什么 SQL 允许执行"
从始至终只有 `validate_readonly_sql` 一处定义，现在由 `/chat` 的 `run_readonly_sql` 工具
入口独占使用，SQL 注入向的验收由 `tests/test_tool_run_readonly_sql.py` 在工具层继续覆盖。
