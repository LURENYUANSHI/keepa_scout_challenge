...
`/eligibility` 单查（5 条规则逐项 pass/fail + ROI）→ 批量查询 → `/chat` 发问
（`build_filter_sql` 工具卡 → 流式回答逐字增长 → 最终表格，注意侧栏标题即首条提问）→
`/refresh` 任务状态（32/32 done）。全程真实 DeepSeek 调用；UPC 页未演示是因为录制时
Keepa 出网不可用（见上方"已知环境限制"），有网环境下的真实 UPC 走查截图见下表。
